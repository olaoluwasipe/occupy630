...
const AttributeAddForm = ({ attribute }) => {
  ...
  const handleSubmit = (e) => {
    ...
    if(attribute?.id) {post(route('apartment.attributes.update'))}
    else {post(route('apartment.attributes.store'))}
  }

  return (
    <form onSubmit={handleSubmit}>
      <div>
        <InputLabel htmlFor="name" value="Attribute Name" />
        <TextInput
            id="name"
            type="text"
            name="name"
